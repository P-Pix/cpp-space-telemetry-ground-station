...
namespace stgs {
namespace {
constexpr long long MillisecondsPerSecond = 1000LL;
...
std::string nowIso8601() {
    ...
    const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count() % MillisecondsPerSecond;
    ...
    oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S")
        << '.' << std::setw(MillisecondFieldWidth) << std::setfill('0') << ms << 'Z';
    return oss.str();
}
